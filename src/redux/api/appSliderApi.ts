...
const appSliderApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        getAppSlider: builder.query<AppSliderListResponse, GetBannersParams | void>({
            query: (params) => {
            ...
            providesTags: ['AppSlider'],
        }),

        addAppSlider: builder.mutation<unknown, FormData>({
            query: (formData) => ({
                url: '/banners/',
                method: 'POST',
                body: formData,
            }),
            invalidatesTags: ['AppSlider'],
        }),

        updateAppSlider: builder.mutation<
            unknown,
        ...
        deleteAppSlider: builder.mutation<unknown, string>({
            query: (id) => ({
                url: `/banners/${id}`,
                method: 'DELETE',
            }),
