...
/** Super admin platform overview chart (filled from API). */
...
const PLATFORM_OVERVIEW_YEAR_MIN = 2021

/** Years for the platform overview year selector (newest first). */
export function getPlatformOverviewYearOptions(): string[] {
    const y = new Date().getFullYear()
    const out: string[] = []
    for (let yy = y; yy >= PLATFORM_OVERVIEW_YEAR_MIN; yy -= 1) {
        out.push(String(yy))
    }
    return out
}
